<#
.SYNOPSIS
Measures client-visible video reaction latency after an input trigger.

.DESCRIPTION
This tool sends an input trigger through either the QEMU native input TCP
channel or the local Windows mouse path, then captures the client window until
a selected region visibly changes. It is intended for GVT Cloud Client
acceptance testing and writes per-trial screenshots plus results.csv.
...
param(
    ...
    [string]$OutDir = "build\latency-captures"
)
